...
app.get('/sketches', async (req, res) => {
  try {
      const sketches = await Sketches_Model.find().populate('architect_id');
      res.json(sketches);
  } catch (err) {
...
// Delete a sketch
app.delete('/delete-sketch/:id', async (req, res) => {
  try {
      await Sketches_Model.findByIdAndDelete(req.params.id);
